local bda_eval_card = eval_card
function eval_card(card, context)
    local ret, post = bda_eval_card(card, context)

    if card.keychains and next(card.keychains) and (context.cardarea == G.jokers or context.card == G.consumeables or context.repetition_only) then
        for k, v in pairs(card.keychains) do
            --sendDebugMessage('Кейчейн вызывает функцию калкулэйт кейчейн')
            local keychain = card:calculate_keychain(context, k)
            if keychain then
                sendDebugMessage('Шото добавилось в ret')
                ret[k] = keychain
            end
        end
    end

    return ret, post
end

local bda_trigger_effects = SMODS.trigger_effects
SMODS.trigger_effects = function(effects, card)
    local ret = bda_trigger_effects(effects, card)
    
    for _, effect_table in ipairs(effects) do
        if card then
            --sendDebugMessage('Карта есть')
            if card.keychains and next(card.keychains) then
                for k, v in pairs(card.keychains) do
                    --sendDebugMessage('Функция калкулэйт эффект вызывается')
                    --for k,v in pairs(ret) do
                    --    sendDebugMessage(k.." : "..tostring(v))
                    --end
                    --for k, v in pairs(effect_table) do
                    --    sendDebugMessage('effect_table key: '..tostring(k)..' val: '..tostring(v))
                    --end
                    SMODS.calculate_effect_table_key(effect_table, k, card, ret)
                end
            end
        end
    end

    return ret
end

local bda_generate_UIBox = Card.generate_UIBox_ability_table
function Card:generate_UIBox_ability_table()
    local full_UI_table = bda_generate_UIBox(self)
    
    if self.keychains and next(self.keychains) then
        full_UI_table.badges[#full_UI_table.badges + 1] = 'bda_keychains'
        for k, v in pairs(self.keychains) do
                local new_info, vars = {}, {}

                new_info.name = G.localization.descriptions.Keychains[k] and G.localization.descriptions.Keychains[k].name or k
                if v.ref.loc_vars and type(v.ref.loc_vars) == 'function' then
                    local info_queue = {}
                    local loc_vars_result = v.ref:loc_vars(info_queue, self)
                    vars = loc_vars_result and loc_vars_result.vars or {}
                    -- добавляем всё что loc_vars положил в info_queue
                    for _, v in ipairs(info_queue) do
                        generate_card_ui(v, full_UI_table)
                end

                localize{
                    type = 'descriptions',
                    key = k,
                    set = 'Keychains',
                    nodes = new_info,
                    vars = vars
                }
                full_UI_table.info[#full_UI_table.info + 1] = new_info
            end
        end
    end
    
    return full_UI_table
end

local bda_card_save = Card.save
function Card:save()
    local cardTable = bda_card_save(self)

    if self.keychains and next(self.keychains) then
        cardTable.keychains = {}
        for k, v in pairs(self.keychains) do
            cardTable.keychains[k] = v.ability
        end
    end

    return cardTable
end

local bda_load_card = Card.load
function Card:load(cardTable, other_card)
    bda_load_card(self, cardTable, other_card)

    if cardTable.keychains then
        self.keychains = {}
        for k, v in pairs(cardTable.keychains) do
            self.keychains[k] = {
                ref = SMODS.Keychains[k],
                ability = v
            }
        end
    end

end